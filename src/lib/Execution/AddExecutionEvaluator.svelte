<script lang="ts">
  ...
  let {
    ...
    tasks: [string, string][];
    handleGenerateEvaluator: Function;
  } = $props();
...
<div class="flex flex-col gap-y-2 relative">
  {#if tasks.length === 0}
    <div class="text-slate-600 italic">
      <img src="triangle-alert.svg" class="w-6 h-6 inline mr-2" alt="warning" />
      No task has been compiled. Click the compile button under "Execution" to compile
      the tasks.
    </div>
  {:else}
    <div
...
<style lang="postcss">
  ...
  .user-description:empty:before {
    content: "Describe what you want to evaluate and we will generate it for you.";
    color: gray;
    pointer-events: none;
  }
  .selected {
