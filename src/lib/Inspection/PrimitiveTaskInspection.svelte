<script lang="ts">
  ...
  function handleDeleteDocInputKey(doc_input_key) {
    ...
    if (
      new_task.execution?.parameters?.feature_key &&
      new_task.execution.parameters.feature_key === doc_input_key
    ) {
  ...
  function handleAddDocInputKey(existing_key) {
    ...
    const new_task = JSON.parse(JSON.stringify(task));
    new_task.doc_input_keys = [...task.doc_input_keys!, existing_key];

    // Find the schema for this key from available_states
    let keySchema = null;
    if (
      new_task.state_input_key &&
      new_task.available_states &&
      new_task.available_states[new_task.state_input_key]
    ) {
      const keyInfo = new_task.available_states[new_task.state_input_key].find(
        (k) => k.key === existing_key
      );
      if (keyInfo) {
    ...
    new_task.input_keys = [
      ...(new_task.input_keys || []),
      { key: existing_key, schema: keySchema },
    ];
  ...
  function handleDeleteStateInputKey() {
    ...
    if (
      old_state_input_key &&
      new_task.available_states &&
      new_task.available_states[old_state_input_key]
    ) {
      const old_state_keys = new_task.available_states[old_state_input_key].map(
        (k) => k.key
      );
      if (new_task.doc_input_keys) {
        new_task.doc_input_keys = new_task.doc_input_keys.filter(
          (k) => !old_state_keys.includes(k)
        );
      }
    }
  ...
  function handleAddStateInputKey(state_key) {
    ...
    if (new_task.available_states && new_task.available_states[state_key]) {
      ...
      const available_keys = new_task.available_states[state_key].map(
        (k) => k.key
      );
      ...
      if (!new_task.doc_input_keys) {
        ...
        if (
          ...
          const old_state_keys = new_task.available_states[
            old_state_input_key
          ].map((k) => k.key);
          new_task.doc_input_keys = new_task.doc_input_keys.filter(
            (k) => !old_state_keys.includes(k)
          );
          new_task.existing_keys = new_task.existing_keys?.filter(
            (k) => !old_state_keys.includes(k)
          );
          new_task.input_keys = new_task.input_keys?.filter(
            (obj) => !old_state_keys.includes(obj.key)
          );
        }
      }
      ...
      if (
        ...
        !new_task.doc_input_keys.includes(available_keys[0])
      ) {
        new_task.doc_input_keys.push(available_keys[0]);
        new_task.existing_keys = [
          ...(new_task.existing_keys || []),
          available_keys[0],
        ];
        new_task.input_keys = [
          ...(new_task.input_keys || []),
          {
            ...
            schema: new_task.available_states[state_key].find(
              (k) => k.key === available_keys[0]
            )?.schema,
          },
        ];
        ...
        new_task.execution.parameters.input_key_schemas =
          new_task.input_keys?.reduce((acc, obj) => {
